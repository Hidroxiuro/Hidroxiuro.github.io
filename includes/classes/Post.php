...
class Post {
	...
	public function submitPost($body, $user_to, $imageName) {
      ...
      if ($check_empty==""){$check_empty = " ";}
		if($check_empty != "") {
			...
			//insert post 
			$query = mysqli_query($this->con, "INSERT INTO posts(body, added_by, user_to, date_added, user_closed, deleted, likes, imagenUrl) 
	...
	public function loadPostsFriends($data, $limit) {
		...
		$page = $data['page']; 
		$userLoggedIn = $this->user_obj->getUsername();///checar esto
		...
		if(mysqli_num_rows($data_query) > 0) {
			...
			while($row = mysqli_fetch_array($data_query)) {
				...
				$imagePath = $row['imagenUrl'];
				$user_toP ="profile.php?profile_username=$row[added_by]";
				...
				if($user_logged_obj->isFriend($added_by)){
					...
					$str .= "<div class='status_post' onClick='javascript:toggle$id()'>
								...
								<div class='posted_by' style='color:#ACACAC;'>
									<a href='$user_toP'> $first_name $last_name </a> $user_to &nbsp;&nbsp;&nbsp;&nbsp;$time_message
									$delete_button
								</div>
								<div id='post_body'>
									$body
